...
do
    ...
    aimbot_settings.torso = settings:Get("aimbot.torso", false)
    ...
    function aimbot.step()
        local body = 'Head'
        if aimbot.torso == true then
            body = 'Torso'
        end
        if aimbot.torso == false then
            body = 'Head'
         end
    ...
    function visuals.step()
        ...
        if credits == nil then
            credits = newdrawing("Text", {
                Text = "Crownbot v7, eve#1542"; -- yes now be happy this is free
...
AimbotToggle:AddLabel({
    Text = "check if alive better be on, retard"
})
...
Hud:AddLabel({
    Text = "eve_trader is a homosexual"
})
